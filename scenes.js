...
class SceneGenerator {
    ...
    insertFavoriteDrugstoreScene() {
        ...
        insertFavoriteDrugstore.enter(async ctx => {
            ...
            await ctx.reply(`Если у вас остались вопросы, то можете ознакомиться с полной информацией по ассортименту в МП (ссылка), на сайте (ссылка) или <a href="${GROUP_URL}">задать вопрос оператору</a>`, {parse_mode: 'HTML', disable_web_page_preview: true, reply_markup: backMainMenu.reply_markup})
            //ctx.scene.leave()
        })
        insertFavoriteDrugstore.action('back', async ctx => {
            ctx.scene.leave()
            await ctx.scene.enter('getDrugStore')
        })
    ...
    deleteFavoriteDrugstoreScene() {
        ...
        deleteFavoriteDrugstore.enter(async (ctx) => {
            ...
            await ctx.reply(`Если у вас остались вопросы, то можете ознакомиться с полной информацией по ассортименту в МП (ссылка), на сайте (ссылка) или <a href="${GROUP_URL}">задать вопрос оператору</a>`, {parse_mode: 'HTML', disable_web_page_preview: true, reply_markup: Markup.inlineKeyboard([Markup.button.callback('🏠 В главное меню', 'mainMenu'),]).reply_markup})
            ctx.scene.leave()
        })
